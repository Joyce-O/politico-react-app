...
export const NewOfficeSchema = Yup.object().shape({
  ...
  ageLimit: Yup.string().required('age is required'),
...
export const NewPartySchema = Yup.object().shape({
  name: Yup.string()
    .matches(/^[a-z\d\s]+$/i, 'symbols are not allowed in party name')
    .min(3,
  ...
  email: Yup.string()
    .email('Invalid email')
    .required('Email is Required'),
  phone: Yup.number().required('A valid phone number is required'),
});
...
export const newCandSchema = Yup.object().shape({
  ...
  party: Yup.number().required('A valid partyId is required'),
});
